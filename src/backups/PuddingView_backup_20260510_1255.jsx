...
const PuddingView = ({ activeTab = 'dashboard', role, setActiveTab, fullProfile }) => {
  ...
  const [maBaMappings, setMaBaMappings] = useState({}); // { maOrg: [baOrg1, baOrg2] }
  ...
  const handleBulkChange = async () => {
    ...
    try {
      ...
      for (const id of selectedBulkProjectIds) {
  ...
  const handleCopyProject = async (sourceProject) => {
    setIsSubmitting(true);
    try {
      const newProject = {
        ...sourceProject,
        id: `PRJ-${Math.random().toString(36).substring(2, 9).toUpperCase()}`,
        name: `【複製】${sourceProject.name}`,
        status: 'requesting',
        created_at: new Date().toISOString(),
        metadata: { ...sourceProject.metadata, copied_from: sourceProject.id }
      };
      // 不要なフィールドの削除
      delete newProject.timestamp; // Profile hack用フィールドがある場合

      await api.createProject(newProject);
      alert('案件を複製しました。案件ボードで内容を確認・編集してください。');
      fetchProjects();
    } catch (err) {
      console.error(err);
      alert('複製に失敗しました。');
  ...
  const handleStationSave = async (isFinal = false) => {
    ...
    try {
      ...
      let stationName = fullProfile?.name || '系列局A'; 
  ...
  const handleAgencyMaterialUpload = async (projectId, stationName) => {
    ...
    input.onchange = async (e) => {
      ...
      setIsSubmitting(true);
      try {
        const path = await api.uploadMaterialFile(file);
        ...
        if (success) {
          alert(`${stationName} の素材をアップロードしました。送信ボタンで確定してください。`);
          fetchProjects();
          if (selectedBoardProject) {
            const res = await api.getStationResponses(selectedBoardProject.id);
            setSelectedProjectResponses(res || []);
  ...
  const handleMaterialDownload = async (projectId, stationName) => {
    try {
      const resp = await api.getStationResponses(projectId);
      const current = resp.find(r => r.station_name === stationName) || {};
      const path = current.response_data?.material_path;
      ...
      // Transition to rewrites after download
  ...
  const handleRewriteUpload = async (projectId, stationName) => {
    const input = document.createElement('input');
    input.type = 'file';
  ...
  const handleRevisedUpload = async (projectId, stationName) => {
    const input = document.createElement('input');
    input.type = 'file';
    input.accept = '.doc,.docx,.pdf,.txt';
    input.onchange = async (e) => {
  ...
  const handleRecordingUpload = async (projectId, stationName) => {
    const input = document.createElement('input');
    input.type = 'file';
  ...
  const handleCreateProject = async () => {
    ...
    try {
      ...
      alert('案件の作成に失敗しました: ' + err.message);
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  useEffect(() => {
    fetchProjects();
    const savedMappings = localStorage.getItem('pudding_ma_ba_mappings');
    if (savedMappings) setMaBaMappings(JSON.parse(savedMappings));
    ...
    const loadStationResponse = async () => {
      if (selectedRequest && role === 'broadcaster') {
        try {
          ...
          const stationName = fullProfile?.name || '系列局A';
          ...
          if (myResponse && myResponse.response_data) {
            ...
            setFormRecordingDate(m.recordingDate || '');
  ...
  const saveMaBaMappings = (newMappings) => {
    setMaBaMappings(newMappings);
    localStorage.setItem('pudding_ma_ba_mappings', JSON.stringify(newMappings));
  };
  
  const renderDashboard = () => {
    const cards = [
      { label: '現在のスポット稼働率', value: '72%', icon: Zap, color: '#ffd93d' },
      { label: '本日公開のプラン', value: '8件', icon: Sun, color: '#fb923c' },
      { label: '空き枠通知', value: '3件', icon: Cloud, color: '#38bdf8' }
    ];
          ...
          <div style={{ display: 'grid', gap: '1rem' }}>
            {isLoading ? (
              ...
              const statusColors = {
                ...
                slots_registration: { label: '枠登録中', color: '#3b82f6' },
                material_check: { label: '素材確認中', color: '#8b5cf6' },
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'board':
        ...
        if (role === 'agency' && selectedBoardProject) {
           ...
           boardItems = projects.map(p => {
              ...
              const response = p.metadata?.[`response_${stationName}`] || {};
              return { 
                 ...p, 
                 sponsor: p.sponsor_name || p.metadata?.sponsor || '未設定', 
                 station: stationName, 
                 status: p.status === 'requesting' ? 'slots' : p.status,
                               ...
                               {col.id === 'materials' && (
                                  role === 'agency' ? (
                                    <div style={{ display: 'flex', gap: '8px' }}>
                                       <button onClick={(e) => { e.stopPropagation(); alert('素材を選択します'); }} style={{ flex: '1', padding: '10px', borderRadius: '12px', backgroundColor: '#f59e0b', color: 'white', border: 'none', fontSize: '11px', fontWeight: '950', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px' }}>
                                          <Upload size={12} /> 素材UP
                                       </button>
                                       <button onClick={(e) => { e.stopPropagation(); handleMaterialUpload(item.projectId || item.id, item.station); }} style={{ flex: '1', padding: '10px', borderRadius: '12px', backgroundColor: '#3E2723', color: 'white', border: 'none', fontSize: '11px', fontWeight: '950', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px' }}>
                                          <Check size={12} /> 送信
                                       </button>
                                    </div>
                                  ) : (
                                    <button 
                                       ...
                                       style={{ 
                                          ...
                                          backgroundColor: item.has_material ? '#3b82f6' : '#94a3b8', 
                                          color: 'white', border: 'none', fontSize: '11px', fontWeight: '950', 
                                          cursor: item.has_material ? 'pointer' : 'not-allowed', 
                                          display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px',
                                          opacity: item.has_material ? 1 : 0.6
                                       }}
                                    >
                                  ...
                                  role === 'agency' ? (
                                    ...
                                    <>
                                       ...
                                       <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                          ...
                                          <div style={{ display: 'flex', gap: '8px' }}>
                                             <button onClick={(e) => { e.stopPropagation(); alert('修正〆切時間を設定します'); }} style={{ flex: '1', padding: '10px', borderRadius: '12px', backgroundColor: 'white', color: '#ef4444', border: '1.5px solid #ef4444', fontSize: '11px', fontWeight: '950', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px' }}>
                                                <Clock size={12} /> 修正〆切
                                             </button>
                                             <button onClick={(e) => { e.stopPropagation(); alert('リライト原稿を送信します'); }} style={{ flex: '1', padding: '10px', borderRadius: '12px', backgroundColor: '#3E2723', color: 'white', border: 'none', fontSize: '11px', fontWeight: '950', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px' }}>
                                                <Check size={12} /> 送信
                                             </button>
                                          </div>
                                          <button 
                                             disabled={!item.has_revised_material}
                                             onClick={(e) => { e.stopPropagation(); alert('修正稿をダウンロードします'); }} 
                                             style={{ 
                                                width: '100%', padding: '10px', borderRadius: '12px', 
             ...
             <div style={{ display: 'grid', gap: '24px' }}>
                ...
                <div style={{ backgroundColor: 'white', borderRadius: '28px', border: '1.5px solid #F1E4C9', overflow: 'hidden', boxShadow: '0 10px 25px rgba(0,0,0,0.03)' }}>
                   <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                      <thead style={{ backgroundColor: '#fcfcfd', borderBottom: '1.5px solid #F1E4C9' }}>
                         <tr>
                            <th style={{ padding: '20px 32px', textAlign: 'left', fontWeight: '950', color: '#8B4513', fontSize: '13px' }}>スポンサー / 案件名</th>
                            <th style={{ padding: '20px 32px', textAlign: 'left', fontWeight: '950', color: '#8B4513', fontSize: '13px' }}>依頼期間</th>
                            <th style={{ padding: '20px 32px', textAlign: 'center', fontWeight: '950', color: '#8B4513', fontSize: '13px' }}>現在の状態</th>
                            <th style={{ padding: '20px 32px', textAlign: 'center', width: '120px' }}>操作</th>
                         </tr>
                      </thead>
                      <tbody>
                         {isLoading ? (
                            <tr><td colSpan={4} style={{ padding: '48px', textAlign: 'center', color: '#94a3b8' }}>読み込み中...</td></tr>
                         ) : filteredCopyProjects.length === 0 ? (
                            <tr><td colSpan={4} style={{ padding: '48px', textAlign: 'center', color: '#94a3b8' }}>該当する案件が見つかりません</td></tr>
                         ) : filteredCopyProjects.map(p => (
                            <tr key={p.id} style={{ borderBottom: '1px solid #F1E4C9', transition: 'background-color 0.2s' }}>
                               <td style={{ padding: '20px 32px' }}>
                                  <div style={{ fontSize: '12px', color: '#8B4513', fontWeight: '800', marginBottom: '4px' }}>{p.sponsor_name}</div>
                                  <div style={{ fontSize: '15px', fontWeight: '950', color: '#3E2723' }}>{p.name}</div>
                               </td>
                               <td style={{ padding: '20px 32px' }}>
                               ...
                               <td style={{ padding: '20px 32px', textAlign: 'center' }}>
                                  <button 
                                     onClick={() => {
                                        if (confirm(`「${p.name}」を複製して新しい案件を作成しますか？`)) {
                                          handleCopyProject(p);
                                        }
                                     }}
      ...
      case 'slot-move-suspended':
        return (
          <PageView title="枠移動・休止" desc="枠の変更操作を行います。" icon={History} color="#EF4444">
             <div style={{ backgroundColor: 'white', border: '1.5px solid #F1E4C9', borderRadius: '24px', padding: '32px' }}>
                <p>登録済み枠の一覧...</p>
             </div>
          </PageView>
        );
  ...
  return (
    <div style={{ padding: '2.5rem', maxWidth: '1400px', margin: '0 auto' }}>
      ...
      {activeModal === 'zone' && (
         <Modal title="依頼ゾーンを設定" onClose={() => setActiveModal(null)} width="400px">
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
               ...
               <div>
                  <label>終了</label>
                  <select value={endHour} onChange={(e) => setStartHour(e.target.value)} style={{ width: '100%', padding: '10px' }}>
                     {Array.from({ length: 24 }).map((_, i) => <option key={i} value={i.toString().padStart(2, '0')}>{i}:00</option>)}
                  </select>
               </div>
            </div>
      ...
      {(activeModal === 'formOADate' || activeModal === 'formMaterialDeadlineLimit') && (() => {
         const year = viewMonth.getFullYear();
         const month = viewMonth.getMonth();
         const daysInMonth = new Date(year, month + 1, 0).getDate();
         const firstDayOffset = new Date(year, month, 1).getDay();
         ...
         const title = isOADate ? "OA日を選択" : "素材搬入予定日を選択";
            ...
            <Modal title={title} onClose={() => setActiveModal(null)} width="400px">
               ...
               <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '8px' }}>
                  ...
                  {Array.from({ length: daysInMonth }).map((_, i) => {
                     ...
                     const isSelected = isOADate ? formOADate === dateStr : formMaterialDeadlineLimit === dateStr;
               ...
               <div style={{ marginTop: '20px', fontSize: '12px', color: '#64748b', textAlign: 'center' }}>
                  {isOADate ? "依頼期間内（" + selectedRequest?.start_date + " 〜 " + selectedRequest?.end_date + "）の日付のみ選択可能です。" : "素材搬入開始（" + (selectedRequest?.metadata?.material_start_date || selectedRequest?.metadata?.material_deadline) + "）以降の日付のみ選択可能です。"}
               </div>
            </Modal>
         );
      })()}
