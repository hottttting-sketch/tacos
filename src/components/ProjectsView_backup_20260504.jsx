...
const ProjectsView = ({ role }) => {
      ...
      if (data && data.length > 0) {
        ...
        const initialProjects = [
          { 
            ...
            area: ['関東', '関西'],
            hearingItems: { checkRefPrice: true, checkAG: true, checkStraightPub: true, checkProposal: true },
            scale: '3000 GRP',
            tg: 'F2 (35-49歳女性)',
            seconds: '15s',
            ag: '15%',
        ...
        localStorage.setItem('tacos_local_projects', JSON.stringify(initialProjects));
        setProjects(initialProjects);
      }
    } catch (e) {
      console.warn('DB fetch failed, using fallback empty state:', e);
      setProjects([]);
    } finally {
      setIsLoading(false);
    }
  ...
  const filteredProjects = projects.filter(p => {
    const isMatchingType = !p.type || p.type === projectType || p.metadata?.type === projectType;
  ...
  return (
    <div className="animate-fade" style={{ padding: '24px', maxWidth: '1200px', margin: '0 auto' }}>
      {activeTab === 'list' ? (
                ...
                {filteredProjects.length === 0 ? (
                  ...
                  <tr key={p.id} onClick={() => handleProjectClick(p)} style={{ borderBottom: '1px solid #f1f3f5', cursor: 'pointer', transition: 'all 0.2s' }} className="hover-row">
                    <td style={{ padding: '20px 24px' }}>
                      ...
                      <div style={{ fontSize: '12px', color: '#94a3b8', marginTop: '2px' }}>{p.sponsor_name || '未設定スポンサー'}</div>
                    </td>
                    <td style={{ padding: '20px 24px' }}>
        ...
        <ProjectMatrix project={selectedProject} onBack={handleBack} />
      )}
...
/* --- 集約・プランニング画面 --- */
const ProjectMatrix = ({ project, onBack }) => {
  ...
  const fetchStations = async () => {
    ...
    try {
      ...
      if (
        ...
        (currentProj.sponsor_name && currentProj.sponsor_name.includes('ああ'))
      ) {
        selectedStations = ['EX'];
      }
  ...
  const getIndexLabels = (proj = project) => {
    ...
    ['F1', 'F2', 'F3', 'M1', 'M2', 'M3', '主婦', 'C', 'T'].filter(ind =>
      (proj.tg || '').includes(ind) ||
      (proj.metadata?.indices || []).includes(ind)
        ...
        ans.spotRows.forEach(sr => {
          if (sr) {
            Object.keys(sr).forEach(k => {
              if (k.startsWith('index_')) {
                extracted.add(k.replace('index_', ''));
              }
            });
          }
        });
  ...
  const getServiceShares = (proj = project) => {
    ...
    if (shares && shares.length > 0) {
      return shares.map(s => `シェア${s.value}%`);
    }
    return ['シェ30%', 'シェ40%'];
  };
         ...
         <div className="animate-fade" style={{ marginBottom: '32px' }}>
            ...
            {selectedPlanningMode === 'ai' && (
               <div className="animate-slide-up" style={{ backgroundColor: '#fff9db', padding: '24px', borderRadius: '24px', display: 'flex', flexDirection: 'column', gap: '16px', border: '1.5px solid #fff3bf' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
                    <button onClick={() => { setIsConfigured(true); alert('各種配分を一覧に設定しました。'); }} style={{ padding: '14px 32px', borderRadius: '12px', backgroundColor: '#f08c00', color: 'white', fontWeight: '900', border: 'none', cursor: 'pointer', boxShadow: '0 4px 12px rgba(240,140,0,0.2)' }}>設定</button>
                  </div>
               </div>
            )}
            ...
            {selectedPlanningMode === 'import' && (
               <div className="animate-slide-up" style={{ backgroundColor: '#e7f5ff', padding: '24px', borderRadius: '24px', display: 'flex', flexDirection: 'column', gap: '16px', border: '1.5px solid #d0ebff' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
                     <button onClick={() => { setIsConfigured(true); alert('各種配分を一覧に設定しました。'); }} style={{ padding: '14px 32px', borderRadius: '12px', backgroundColor: '#4338ca', color: 'white', fontWeight: '900', border: 'none', cursor: 'pointer', boxShadow: '0 4px 12px rgba(67,56,202,0.2)' }}>設定</button>
                  </div>
               </div>
            )}
            ...
            {selectedPlanningMode === 'manual' && (
               <div className="animate-slide-up" style={{ backgroundColor: '#ebfbee', padding: '24px', borderRadius: '24px', border: '1.5px solid #d3f9d8', display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  <div style={{ display: 'flex', gap: '24px' }}>
                     {['発注金額', '発注PRP', 'サービスGRP', '枠追加'].map(item => (
                        <label key={item} style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
                           <input 
                             type="checkbox" 
                             checked={checkedPlanningItems.includes(item)}
                             onChange={(e) => {
                               if (e.target.checked) {
                                 setCheckedPlanningItems(prev => [...prev, item]);
                               } else {
                                 setCheckedPlanningItems(prev => prev.filter(p => p !== item));
                               }
                             }}
                  ...
                  <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
                    <button onClick={() => { setIsConfigured(true); alert('各種配分を一覧に設定しました。'); }} style={{ padding: '14px 32px', borderRadius: '12px', backgroundColor: '#087f5b', color: 'white', fontWeight: '900', border: 'none', cursor: 'pointer', boxShadow: '0 4px 12px rgba(8,127,91,0.2)' }}>設定</button>
                  </div>
               </div>
            )}
                    ...
                    <div style={{ flex: 1, overflowY: 'auto', paddingRight: '8px' }}>
                      ...
                      {selectedModalStationName && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                          ...
                          {/* 1. 基本事項 & 発注条件概要 */}
                          <div style={{ backgroundColor: '#f8fafc', padding: '20px', borderRadius: '20px', border: '1.5px solid #f1f5f9' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '14px' }}>
                               <IconWrapper color="rgba(230,0,18,0.08)" iconColor="var(--tacos-red)" size={24} borderRadius={6}><Icons.FileText size={13} /></IconWrapper>
                               <span style={{ fontSize: '14px', fontWeight: '950', color: '#1e293b' }}>基本事項・発注条件 ({selectedModalStationName})</span>
                            </div>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '12px 24px' }}>
                              <div><span style={{ fontSize: '12px', color: '#64748b', fontWeight: '800' }}>案件名:</span> <span style={{ fontSize: '13px', color: '#1e293b', fontWeight: '900' }}>{project.name}</span></div>
                              <div><span style={{ fontSize: '12px', color: '#64748b', fontWeight: '800' }}>スポンサー:</span> <span style={{ fontSize: '13px', color: '#1e293b', fontWeight: '900' }}>{project.sponsor_name}</span></div>
                              <div><span style={{ fontSize: '12px', color: '#64748b', fontWeight: '800' }}>実施エリア:</span> <span style={{ fontSize: '13px', color: '#1e293b', fontWeight: '900' }}>{project.area && project.area.join(', ')}</span></div>
                              <div><span style={{ fontSize: '12px', color: '#64748b', fontWeight: '800' }}>放送期間:</span> <span style={{ fontSize: '13px', color: '#1e293b', fontWeight: '900' }}>{project.start_date}〜</span></div>
                              <div>
                                <span style={{ fontSize: '12px', color: '#64748b', fontWeight: '800' }}>発注金額:</span> 
                                <span style={{ fontSize: '15px', color: 'var(--tacos-red)', fontWeight: '950' }}>
                                  {parseInt((tableInputValues[`${selectedModalStationName}_発注金額`] || '').replace(/[^0-9]/g, ''))?.toLocaleString() || '-'} 円
                                </span>
                              </div>
                              <div>
                                <span style={{ fontSize: '12px', color: '#64748b', fontWeight: '800' }}>発注PRP:</span> 
                                <span style={{ fontSize: '15px', color: 'var(--tacos-red)', fontWeight: '950' }}>
                                  {tableInputValues[`${selectedModalStationName}_発注PRP`] || '-'} PRP
                                </span>
                              </div>
                            </div>
                          </div>
                          ...
                          {/* 2. 回答一覧のINDEXまでの各種項目 */}
                          <div style={{ border: '1.5px solid #f1f5f9', borderRadius: '20px', padding: '20px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '14px' }}>
                               <IconWrapper color="rgba(2,132,199,0.08)" iconColor="#0284c7" size={24} borderRadius={6}><Icons.Dashboard size={13} /></IconWrapper>
                               <span style={{ fontSize: '14px', fontWeight: '950', color: '#1e293b' }}>回答項目（INDEXまで）</span>
                            </div>
                            
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '14px 20px' }}>
                              {activeColumns.filter(col => !col.key.startsWith('service_') && !col.key.startsWith('checkStraight') && !col.key.startsWith('checkInterview') && !col.key.startsWith('checkTalk') && !col.key.startsWith('checkPre') && !col.key.startsWith('checkProgram') && !col.key.startsWith('checkPlanning') && !col.key.startsWith('checkProposal')).map(col => {
                                const normField = (col.field || '').replace(/\s+/g, '');
                                let cellVal = '-';
                                const foundSt = stations.find(s => s.name === selectedModalStationName);
                                if (foundSt && foundSt.responses) {
                                  const foundKey = Object.keys(foundSt.responses).find(k => 
                                    k.replace(/\s+/g, '') === normField ||
                                    k.replace(/\s+/g, '') === normField.replace('index_', '') ||
                                    k === col.field
                                  );
                                  if (foundKey) cellVal = foundSt.responses[foundKey];
                                }
                                return (
                                  <div key={col.key} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 14px', backgroundColor: '#fafbfc', borderRadius: '12px', border: '1px solid #f1f5f9' }}>
                                    <span style={{ fontSize: '12px', color: '#64748b', fontWeight: '800' }}>{col.label}:</span>
                                    <span style={{ fontSize: '13px', color: '#1e293b', fontWeight: '900' }}>{cellVal || '-'}</span>
                                  </div>
                                );
                              })}
                            </div>
